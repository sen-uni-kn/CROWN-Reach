...
def run_attack_simulation(config_path):
    ...
    def odefunc(t, y):
        locals_dict = {
            config["initialSet"][i]["name"]: y[i].item() for i in range(len(y))
        }
        input_vars = (
        ...
        for i in range(config["num_nn_output"]):
            locals_dict[f"u{i+1}"] = (
                adjusted_control_outputs[i]
                if adjusted_control_outputs.ndim > 0
                else adjusted_control_outputs.item()
            )
    ...
    def evaluate_constraints(trajectory, constraints):
        ...
        for constraint in constraints:
            ...
            for point in trajectory:
                locals_dict = {
                    config["initialSet"][i]["name"]: point[i].item()
